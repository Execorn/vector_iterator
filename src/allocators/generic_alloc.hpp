...
public:
    const GenericMalloc* getInstance() {
...
private:
    ...
    static GenericMalloc* m_instance;
...
private:
    ...
    inline size_t alignBytes(const size_t n_bytes) const {
        /* sizeof(data_t) = 4 or 8 -> 7 is 111 -> ~7 is 000 -> last 3 bits will be zeros*/
        return ~(sizeof(data_t) - 1) & (n_bytes + sizeof(data_t) - 1);
    }

    inline size_t allocationSize(const size_t n_bytes) const {
        // adding header (Chunk) size BUT without Chunk().m_data (it's part of user's data)
    ...
    Chunk* shiftToHeader(const data_t* chunk_ptr) const {
        if (chunk_ptr == nullptr) {
            throw std::invalid_argument("chunk pointer is null");
        }

        return (Chunk*) ((uint8_t*) chunk_ptr + sizeof(std::declval<Chunk>().m_data) - sizeof(Chunk));
    }

    Chunk* getFreeChunk(const size_t n_bytes) const {
        if (n_bytes == 0) {
            return nullptr;
        }
        ...
        switch (m_mem_mode) {
            ...
            case MemoryManagement::best_fit_search: {
                ...
                throw std::runtime_error("Don't use best-fit. Rumors say cats hate it...");
            }
    ...
    Chunk* memFirstFit(const size_t n_bytes) const {
        if (n_bytes == 0) {
            return nullptr;
        }
    ...
    Chunk* memNextFit(const size_t n_bytes) const {
        if (n_bytes == 0) {
            return nullptr;
        }
    ...
    Chunk* splitChunk(Chunk* cur_chunk, const size_t n_bytes) const {
        if (isSplittable(cur_chunk, n_bytes)) {
            size_t splitted_chunk_old_size = cur_chunk->m_size;
            cur_chunk->m_size = n_bytes;

            Chunk* next_chunk_pointer = (Chunk*) (((uint8_t*) cur_chunk) + allocationSize(0) + cur_chunk->m_size);
            next_chunk_pointer->m_size = splitted_chunk_old_size - cur_chunk->m_size - allocationSize(0);
    ...
    inline bool isSplittable(const Chunk* cur_chunk, const size_t n_bytes) const {
        // free space left for the rest part of out empty block is not allowed to be less than 16
        return cur_chunk->m_size - n_bytes - allocationSize(0) >= SPLIT_RATE_MIN_BYTES;
    }

    Chunk* allocateFromList(Chunk* cur_chunk, const size_t n_bytes) const {
    ...
    inline bool isCoalesceableNext(const Chunk* chunk_ptr) const {
        return chunk_ptr->m_next != nullptr && chunk_ptr->m_next->m_used == false;
    }

    inline bool isCoalesceablePrev(const Chunk* chunk_ptr) const {
    ...
    Chunk* coalesceChunk(Chunk* cur_chunk) {
        if (cur_chunk == nullptr || cur_chunk->m_next == nullptr) {
            throw std::invalid_argument("coalesce chunks is broken, report it to devs");
        }    
    ...
    Chunk* freeList(const size_t n_bytes) const {
        if (n_bytes <= 0) {
            return nullptr;
        } 
